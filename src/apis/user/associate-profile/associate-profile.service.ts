...
import { Associate } from './associate-profile.schema';
...
export class AssociateProfileService {
  ...
  constructor(
    ...
    private readonly model: Model<Associate>,
    @InjectModel('Auth')
    private readonly authModel: Model<Auth>,
    private mailService: MailService,
  ) {}

  async addAssociate(payload) {
    const profile = await new this.model({
      ...payload,
      timeStamp: [{ date: new Date() }],
    }).save();
    await this.mailService.associateVerification(
    ...
    return {
      header: {
        code: 200,
        status: 'success',
        message: 'Associate Profile Added',
      },
      data: {
  ...
  async getAssociateById(_id) {
    const result = await this.model.findOne({ _id: _id });
    if (result) return result;
    else throw new NotFoundException('Associate Not Found .');
  }

  async getAssociateList(payload) {
    let queryResult: any;
  ...
  async getAssociateApproved(payload) {
    let profile: any;
    let authProfile: any;
    ...
    return {
      header: {
        code: 200,
        status: 'success',
        message: `Associate Profile Approved`,
      },
      data: authProfile,
    };
  }
  ...
  async updateAssociate(payload) {
    ...
    return {
      header: {
        code: 200,
        status: 'success',
        message: `Associate ${!queryResult ? 'Not ' : ''}Updated`,
      },
      data: {
        _id: payload._id,
        ...payload,
  ...
  async deleteAssociate(_id: string) {
    ...
    return {
      header: {
        code: 200,
        status: 'success',
        message: `Associate ${!isDeleted ? 'Not ' : ''}Deleted`,
      },
      data: {
        _id: _id,
      },
